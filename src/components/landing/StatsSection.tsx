...
export default function StatsSection() {
  return (
    <section aria-labelledby={HEADING_ID} className="bg-surface-primary">
        ...
        <dl className="grid grid-cols-3 gap-6 sm:gap-8">
          {STATS.map(({ label, value, unit }) => (
            <div key={label} className="text-center">
              <dd className="text-3xl font-bold tabular-nums text-accent sm:text-4xl">
                {value.toLocaleString()}
                <span className="ml-1 text-base font-medium">{unit}</span>
              </dd>
              <dt className="mt-1 text-sm text-text-secondary">{label}</dt>
            </div>
          ))}
        </dl>
      </div>
